...
MAX_BODY = 24000
RATE_LIMIT_FALLBACK = timedelta(minutes=15)


def _parse_retry_after(exc: HttpError) -> datetime | None:
    text = (exc.content or b"").decode("utf-8", errors="replace")
    match = re.search(r"Retry after ([0-9T:\.\-Z]+)", text, re.IGNORECASE)
    if not match:
        return None
    raw = match.group(1)
    try:
...
def map_gmail_http_error(exc: HttpError) -> DomainError:
    ...
    if status == 429:
        retry_after = _parse_retry_after(exc) or (utcnow() + RATE_LIMIT_FALLBACK)
        return DomainError(
            "Gmail ograniczył zapytania (limit). Synchronizacja wznowi się automatycznie za kilka minut.",
            429,
            retry_after=retry_after,
        )
    if status in {401, 403}:
        return DomainError("Połączenie z Gmail wygasło. Połącz konto ponownie.", 503)
...
class GmailService:
    ...
    def _email_from_credentials(self, credentials) -> str:
        ...
        raise DomainError("Nie udało się odczytać adresu skrzynki z Google.", 502)
